...
public class Contender extends Personnage {
		...
		if (beings.livingContenders <= 1) {
			...
			if (vie <= 0 || currentMap.z.equals(Zone.EAU)) {
				if(currentMap.z.equals(Zone.EAU)){
					System.out.println("Contender mort noy� en "+x+","+y);
				}
				else if(energie <= 0){
					System.out.println("Contender mort d'�puisement en "+x+","+y);
				}
				else {
	...
	private void seSoigner(Soin soin) {
		...
		closestDangerousEnemy = null;//on s'est soign�, on devrait �tre capable de se taper
		energie--;
		if (energie < 0)
			energie = 0;
	}
